...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"1px solid #C7C7C7","padding":"0","margin":"10px 0 0 auto","alignItems":"center","display":"flex","justifyContent":"center","overflow":"hidden","borderRadius":"30px","flexWrap":"wrap","background":"#fff","width":"100%","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 0"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>车牌号：</div>
	  ...
	  <el-button v-if="btnAuth('weiguigonggao','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 0 0 10px","color":"#fff","borderRadius":"30px","background":"#80BBCA","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/weiguigonggaoAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"width":"100%","padding":"0 0","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
			<div :style='{"border":"1px solid #758A99","padding":"30px","margin":"0","alignItems":"center","background":"#fff","display":"flex","width":"50%","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<div :style='{"width":"270px","padding":"0 0 50px","overflow":"hidden","height":"270px"}'>
					<img @click.stop="imgPreView(item.cheliangtupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.cheliangtupian && item.cheliangtupian.substr(0,4)=='http'" :src="item.cheliangtupian.split(',')[0]" class="image" />
					<img @click.stop="imgPreView(baseUrl + (item.cheliangtupian?item.cheliangtupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.cheliangtupian?item.cheliangtupian.split(',')[0]:'')" class="image" />
				</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`weiguigonggao/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/weiguigonggaoDetail', query: params});
      },
