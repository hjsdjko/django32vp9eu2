...
<div :style='{"width":"100%","padding":"20px 10%","margin":"10px auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none"}' label="违规图片" v-if="type!='cross' || (type=='cross' && !ro.weiguitupian)" prop="weiguitupian">
            <file-upload
            tip="点击上传违规图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none"}' class="upload" v-else label="违规图片" prop="weiguitupian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.weiguitupian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none"}' label="违规备注" prop="weiguibeizhu">
            <el-input v-model="ruleForm.weiguibeizhu" 
                placeholder="违规备注" clearable :disabled=" false  ||ro.weiguibeizhu"></el-input>
          </el-form-item>
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='weiguitupian'){
              this.ruleForm.weiguitupian = obj[o].split(",")[0];
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`weiguijilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('weiguijilu/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`weiguijilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      weiguitupianUploadChange(fileUrls) {
          this.ruleForm.weiguitupian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
